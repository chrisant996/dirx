...
static int ParseColorRule(const WCHAR* in, StrW& value, ColorRule& rule, int level, Error& e)
{
    ...
    if (num_attr == 1 && !(rule.m_not_attr & not_attr_mask) && !rule.m_not_flags && rule.m_patterns.empty())
    {
        ...
        {
            const WCHAR* pct = wcschr(value.Text(), '%');
            if (pct)
            {
                value.SetEnd(pct);
                const int num = _wtoi(value.Text());
                if (num <= 0)
                    s_hidden_opacity = 0;
                else
                {
                    s_hidden_opacity = clamp(double(num) / 100, 0.3, 0.9);
                    SetColorString(ColorIndex(ci), nullptr);
                }
            }
        }
        return 0;
...
const WCHAR* LookupColor(const WCHAR* name, DWORD attr, unsigned short mode)
{
    ...
    else
    {
        ...
        {
            if (s_color_strings[ciHidden])
                ci = ciHidden;
            else if (s_hidden_opacity > 0)
                hidden_opacity = true;
        }
    ...
    for (const auto& rule : s_color_rules)
    {
        ...
        // Match found!
        const WCHAR* ret = rule.m_color.Text();
...
const WCHAR* GetIconColor(const WCHAR* color)
{
    ...
    COLORREF rgb = RgbFromColor(color, RgbFromColorMode::PreferBackground);
    if (rgb == 0xffffffff)
        return color;
